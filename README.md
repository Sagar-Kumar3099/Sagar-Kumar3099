# Hi there! 👋 I'm Sagar Kumar  

🚀 **Aspiring Full-Stack Web Developer** | 🎓 **Lifelong Learner**  

Welcome to my GitHub profile! Here, I showcase projects, collaborate on interesting ideas, and share my journey in tech.  

---

## 🛠️ Skills & Tools  

### Languages  
- **Frontend:** HTML, CSS, JavaScript (React.js)  
- **Backend:** Node.js, Express.js  
- **Database:** MongoDB    

### Tools & Platforms  
- Git & GitHub  
- VS Code  
- Postman (API Testing)  
---

## 📘 Education  

- Computer Science and Engineering
  Guru Gobind Singh Educational Society's Technical Campus, Bokaro, Jharkhand
  2021-2024 

---

## 🌟 Projects  

Here are a few of my favorite projects. Check them out below:  

1. **[Used Car Price Prediction Model](https://github.com/yourusername/used-car-price-prediction)**  
   Predicts vehicle prices based on condition, mileage, and other features using ML algorithms.  

2. **[Loan Application Platform](https://github.com/yourusername/loan-app-platform)**  
   A MERN stack-based app for users to apply for and manage loans.  

3. **[Portfolio Website](https://sagar-kumar3099.github.io/Sagar-Kumar3099github.io/)**  
   A personal portfolio to showcase my skills and projects, built with React.js.  

---

## 🌱 Currently Learning  

- Advanced React.js (State Management with Redux and Context API)  
- RESTful APIs and GraphQL  
- Deployment tools (Docker, AWS, Vercel)  

---

## 📫 How to Reach Me  

- **Email:** sagarzack2003@gmail.com  
- **LinkedIn:**  https://www.linkedin.com/in/sagar-kumar3099/
- **Portfolio:** https://sagar-kumar3099.github.io/Sagar-Kumar3099github.io/ 

---

## 🌐 Fun Fact  

I’m always excited to learn about new technologies and collaborate on innovative projects. Outside of coding, I enjoy [hobby, e.g., gaming, hiking, photography].  

---

⭐️ **Feel free to explore my repositories and connect with me for collaborations!**  
